...
it('handles UnableToCheckFileExistence with S3Exception as previous when checking file existence', function () {
    $path = 'avatars/avatar.jpg';
    ...
    Log::shouldReceive('warning')->once()->with(
        'S3 error checking resource file existence in storage',
...
it('handles UnableToCheckFileExistence without S3Exception as previous when checking file existence', function () {
    $path = 'avatars/avatar.jpg';
    $resource = Resource::factory()->create([
        'type' => 'avatar_image',
        'file_path' => $path,
    ...
    Log::shouldReceive('warning')->once()->with(
        'Flysystem error checking resource file existence in storage',
        Mockery::on(function ($context) {
...
it('handles direct S3Exception when checking file existence', function () {
    $path = 'planets/video.mp4';
    $resource = Resource::factory()->create([
        'type' => 'planet_video',
        'file_path' => $path,
...
it('handles generic exception when checking file existence', function () {
    $path = 'avatars/avatar.jpg';
    $resource = Resource::factory()->create([
        'type' => 'avatar_image',
        'file_path' => $path,
    ...
    Log::shouldReceive('warning')->once()->with(
        'Error checking resource file existence in storage',
        Mockery::on(function ($context) {
            return isset($context['error']) && $context['error'] === 'Generic error';
        })
    );
...
it('uses correct disk based on resource type', function () {
    ...
    // Test planet_video uses video-generation disk config (defaults to s3)
    $resource = Resource::factory()->create([
        'type' => 'planet_video',
        'file_path' => $path,
    ]);

    // Should use s3 disk (default for video-generation)
    expect($resource->file_url)->toBe(Storage::disk('s3')->url($path));
});

it('scopeWithMatchingTags returns all when tags become empty after normalization', function () {
    Resource::factory()->create(['tags' => ['man', 'casual']]);
    Resource::factory()->create(['tags' => ['woman']]);

    // Pass tags that will be filtered out after normalization (empty strings, whitespace)
    $matching = Resource::withMatchingTags(['   ', '', "\t"])->get();

    // Should return all resources (empty tags = no filter)
    expect($matching)->toHaveCount(2);
});
